		...
		<div class="row">
			<div class="col-md-6 offset-3">
				...
				<form action="{{ url('mydata/'.$user->id) }}" method="post" enctype="multipart/form-data">
					...
					<div class="form-group">
						<input type="text" class="form-control @error('document_type') is-invalid @enderror" name="document_type" value="{{ old('document_type', $user->document_type) }}" placeholder="Tipo de Documento">
						@error('document_type')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
					...
					<div class="form-group">
						<input type="text" class="form-control @error('num_document') is-invalid @enderror" name="num_document" value="{{ old('num_document', $user->num_document) }}" placeholder="Número de Documento">
						@error('num_document')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
